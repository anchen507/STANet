...
def decomposition(x,y):
    ...
    res2 = tf.tile(y,[1,1,1,3])
    #print("res2 shape###############")
    print(res2.get_shape())
    for radius in radiux:
        for eps in eps_list:
            LF = guided_filter(res2, x,radius, eps,nhwc=True)
            LF_list.append(LF)
            #HF_list.append(x - LF)
    LF = tf.concat(LF_list, axis=-1)
    #HF = tf.concat(HF_list, axis=-1)
    return LF
...
def process_command_args(arguments):
    ...
    for args in arguments:
        ...
        if args.startswith("w_bright"):
            w_bright = float(args.split("=")[1])
    ...
    return phone, batch_size, train_size, learning_rate, num_train_iters, \
            w_content, w_color, w_texture, w_tv,w_bright,\
