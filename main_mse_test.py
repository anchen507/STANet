...
from model_mse_test import lowlight_enhance
from utils import *
...
parser.add_argument('--patch_size', dest='patch_size', type=int, default=100, help='patch size')
parser.add_argument('--start_lr', dest='start_lr', type=float, default=0.0001, help='initial learning rate for adam')
parser.add_argument('--eval_every_epoch', dest='eval_every_epoch', default=20, help='evaluating and saving checkpoints every #  epoch')
#parser.add_argument('--checkpoint_dir', dest='ckpt_dir', default='./test-checkpoint/20.75', help='directory for checkpoints')
parser.add_argument('--checkpoint_dir', dest='ckpt_dir', default='checkpoint/mseOnly', help='directory for checkpoints')

parser.add_argument('--sample_dir', dest='sample_dir', default='./sample_LH', help='directory for evaluating outputs')

parser.add_argument('--save_dir', dest='save_dir', default='./test_results/TV_01/', help='directory for testing outputs')
parser.add_argument('--test_dir', dest='test_dir', default='./eval15', help='directory for testing inputs')
...
def lowlight_train(lowlight_enhance):
    ...
    eval_low_data_name = glob('eval15/low/*.*')
    eval_high_data_name = glob('eval15/high/*.*')
    ...
    lowlight_enhance.train(train_low_data, train_high_data, eval_low_data, eval_high_data, batch_size=args.batch_size, patch_size=args.patch_size, epoch=args.epoch, lr=lr, sample_dir=args.sample_dir, ckpt_dir=os.path.join(args.ckpt_dir, 'Decom_total_LH'), eval_every_epoch=args.eval_every_epoch, train_phase="Decom_total_LH")
...
def lowlight_test(lowlight_enhance):
    ...
    test_low_data_name = glob(os.path.join(args.test_dir) +'/low'+ '/*.*')
    test_high_data_name = glob(os.path.join(args.test_dir)  +'/high'+ '/*.*')
    test_low_data = []
    test_high_data = []
