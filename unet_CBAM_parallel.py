...
def low_net(input_image):
    input_L=utils_best2.blur(input_image)#高斯模糊
    #misc.imsave('results/' + str(phone)+ "_" + str(idx) + '_unet_connect_' + str(i) + '.jpg', before_after)
    enhanced_L=unet_2.unet(input_L)
    #enhanced_L=unet_2.unet(input_image)
    return enhanced_L
def low_net2(input_image):
    #input_L=utils_best.blur(input_image)#高斯模糊
    #misc.imsave('results/' + str(phone)+ "_" + str(idx) + '_unet_connect_' + str(i) + '.jpg', before_after)
    enhanced_L=model_best.low_net(input_image)
    #enhanced_L=unet_2.unet(input_image)
    return enhanced_L
def high_net(input_image): 
    input_H=tf.subtract(input_image,utils_best2.blur(input_image))#高频图像
    with tf.variable_scope("generatorH"):
        ...
        ch4 = tf.nn.relu(_instance_norm(conv2d(ch3, Wh4) + bh4))+ch2
        ...
        ch6 = conv2d(ch5, Wh6) + bh6+input_H
        #ch6 = conv2d(ch5, Wh6) + bh6+input_image
        enhanced_H=ch6
    return enhanced_H
...
def fusion(input_image):
    with tf.variable_scope("fusion"):
        ...
        cf1 = tf.nn.relu(conv2d(feature_attention, Wf1) + bf1)
        # residual 1
