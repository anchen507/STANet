...
def low_net_guided(input_image,edge_image):
    ...
    enhanced_L=unet_more_concat.unet2(input_L)
    return enhanced_L
def low_net(input_image):
    #input_L=utils_guided_filter.decomposition(input_image,edge_image)
    input_L=utils_best.blur(input_image)#高斯模糊
    #resized_input = tf.image.resize_images(input_L, [50, 50], method=tf.image.ResizeMethod.BICUBIC)#下采样，三次插值
    #with tf.variable_scope("generatorL"):
    enhanced_L=unet_more_concat.unet2(input_L)
    return enhanced_L
def low_net2(input_image):
    input_L=utils_best.blur(input_image)#高斯模糊
    #resized_input = tf.image.resize_images(input_L, [50, 50], method=tf.image.ResizeMethod.BICUBIC)#下采样，三次插值
    #with tf.variable_scope("generatorL"):
    enhanced_L=unet_more_concat.unet3(input_L)
    return enhanced_L

def low_net3(input_image):
    input_L=utils_best.blur(input_image)#高斯模糊
    input_edge=utils_best.tv_grad(input_image)
    input_three1=tf.concat([input_image,input_L],-1)
    input_three=tf.concat([input_three1,input_edge],-1)
    #resized_input = tf.image.resize_images(input_L, [50, 50], method=tf.image.ResizeMethod.BICUBIC)#下采样，三次插值
    #with tf.variable_scope("generatorL"):
    enhanced_L=unet_more_concat.unet3(input_three)
    return enhanced_L

def high_net(input_image):
    input_H=tf.subtract(input_image,utils_best.blur(input_image))#高频图像
    with tf.variable_scope("generatorH"):
        Wh1 = weight_variable([3, 3, 3, 64], name="Wh1"); bh1 = bias_variable([64], name="bh1");
        ch1 = tf.nn.relu(conv2d(input_image, Wh1) + bh1)
...
def high_net2(input_image):
    ...
    with tf.variable_scope("generatorH"):
        ...
        # Final
        Wh6 = weight_variable([3, 3, 64, 3], name="Wh6"); bh6 = bias_variable([3], name="bh6");
        ch6 = conv2d(ch3, Wh6) + bh6
        enhanced_H=ch6
    return enhanced_H
def high_net3(input_image):
    input_L=utils_best.blur(input_image)#高斯模糊
    input_edge=utils_best.tv_grad(input_image)
    input_three1=tf.concat([input_image,input_L],-1)
    input_three=tf.concat([input_three1,input_edge],-1)
    with tf.variable_scope("generatorH"):
        Wh1 = weight_variable([3, 3, 7, 64], name="Wh1"); bh1 = bias_variable([64], name="bh1");
        ch1 = tf.nn.relu(conv2d(input_three, Wh1) + bh1)
        ...
        enhanced_H=ch6
    return enhanced_H
def fusion(input_image):
    with tf.variable_scope("fusion"):
        feature_L=low_net(input_image)
        print(feature_L.get_shape())
        feature_H=high_net(input_image)
...
def fusion3(input_image):
    with tf.variable_scope("fusion3"):
        feature_L=low_net2(input_image)
        print(feature_L.get_shape())
        feature_H=high_net2(input_image)
        print(feature_H.get_shape())
        feature_F=tf.concat([feature_L,feature_H],-1)
        Wf1 = weight_variable([3, 3, 6, 64], name="Wf1"); bf1 = bias_variable([64], name="bf1");
        cf1 = tf.nn.relu(conv2d(feature_F, Wf1) + bf1)
...
def fusion4(input_image):
    with tf.variable_scope("fusion3"):
        feature_L=low_net3(input_image)
        print(feature_L.get_shape())
        feature_H=high_net3(input_image)
        print(feature_H.get_shape())
        feature_F=tf.concat([feature_L,feature_H],-1)
        Wf1 = weight_variable([3, 3, 6, 64], name="Wf1"); bf1 = bias_variable([64], name="bf1");
        cf1 = tf.nn.relu(conv2d(feature_F, Wf1) + bf1)
...
def fusion2(input_image):
    with tf.variable_scope("fusion2"):
        feature_L=low_net(input_image)
        feature_H=high_net(input_image)
        feature_F=tf.concat([feature_L,feature_H],-1)
        Wf1 = weight_variable([3, 3, 6, 64], name="Wf21"); bf1 = bias_variable([64], name="bf21");
        cf1 = tf.nn.relu(conv2d(feature_F, Wf1) + bf1)
        # residual 1
        Wf2 = weight_variable([3, 3, 64, 64], name="Wf22"); bf2 = bias_variable([64], name="bf22");
        cf2 = tf.nn.relu(_instance_norm(conv2d(cf1, Wf2) + bf2))

        Wf3 = weight_variable([3, 3, 64, 64], name="Wf23"); bf3 = bias_variable([64], name="bf23");
        cf3 = tf.nn.relu(_instance_norm(conv2d(cf2, Wf3) + bf3)) + cf1
        # Final
        Wf4 = weight_variable([3, 3, 64, 3], name="Wf24"); bf4 = bias_variable([3], name="bf24");
        cf4 = conv2d(cf3, Wf4) + bf4
        enhanced_out=tf.add(input_image,cf4)
        enhanced_out=tf.divide(input_image,enhanced_out)
    return enhanced_out
